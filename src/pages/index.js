...
class App extends Component {
  ...
  isFilteredMobile = (data, project) => {
    const node = data.allSanityProject.edges[this.state.activePage].node;
    if (this.state.selectedOption === 'alle') {
      if (this.state.authorOption === 'alle') {

        return <div className="frontPageContainer">
          <img style={{ width: '100%', height: '350px', maxWidth: '100%', maxHeight: '50%' }} src={node.image.asset.url} alt={node?.title}/>
  ...
  desktopContent = (data) => {
    const project = data.allSanityProject.edges[this.state.activePage].node;
